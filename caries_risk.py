...
def assess_risk(answers, protective, disease_indicators):
    high_risk = any(answers) or any(disease_indicators)  # если есть хотя бы один фактор риска или индикатор
    moderate_risk = any(protective) and not high_risk    # если есть защитные факторы, но нет высокого риска
    if any(disease_indicators) or any(answers):
        return "Высокий"
    elif moderate_risk:
        return "Средний"
    else:
...
if age_group == "0–5 лет":
    ...
    q2 = st.checkbox("Семья испытывает трудности (бедность, низкая медицинская грамотность)")
    q3 = st.checkbox("Ребёнок часто (более 3 раз в день) употребляет сладости или сладкие напитки")
    ...
    p3 = st.checkbox("Ребёнку наносили фтор местно у врача")
    p4 = st.checkbox("Есть постоянный стоматолог/регулярные визиты")

    st.subheader("Индикаторы заболевания")
    d1 = st.checkbox("Есть начальные (белые пятна) кариозные поражения")
    d2 = st.checkbox("Есть явные кариозные поражения")
    d3 = st.checkbox("Недавно были удалены зубы из-за кариеса или есть пломбы")
...
# ---------------- Анкета для 6+ лет ----------------
    ...
    st.subheader("Факторы риска (социальные/поведенческие/медицинские)")
    q1 = st.checkbox("Есть длительные социальные трудности (бедность, низкая медицинская грамотность)")
    ...
    p2 = st.checkbox("Зубы чистятся фторсодержащей пастой")
    p3 = st.checkbox("Наносили фтор местно у врача")
    p4 = st.checkbox("Есть постоянный стоматолог/регулярные визиты")
